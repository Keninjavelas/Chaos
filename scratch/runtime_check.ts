...
import { startSession, endSession } from "@/archive/session/sessionTracker";
...
async function runChecks() {
  ...
  // 15s idle should add observation via idle detection module (but we only test degradation)
  ...
  // Force trigger a rare event directly (should respect default cooldowns)
  ...
  // 6. Metrics & Flags
  // Derive corruption flags from degradation level
  ...
  // 7. State Restoration (already covered by persistence test)
